...
import {
  ...
  NETWORK_ID_ESPACE_MAINNET,
  NETWORK_ID_ESPACE_TESTNET,
} from '../constants'
...
const useCurrentNetwork = () => {
  ...
  const network = useMemo(() => {
    ...
    const networkId = NETWORK_ID_CORE_MAINNET

    let url =
      //   poolConfig[isTestNetEnv() ? 'testnet' : 'mainnet'][currentSpace].RPC
      poolConfig['mainnet'][currentSpace].RPC
    ...
    return {
      url,
      networkId,
      name: poolConfig[isTestNetEnv() ? 'testnet' : 'mainnet'][currentSpace]
        .name,
    }
  }, [currentSpace])
